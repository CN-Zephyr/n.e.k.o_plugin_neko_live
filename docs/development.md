...
- **观众画像治理**：长期档案只保存基础身份 / 计数和安全派生印象（偏好标签、常聊话题、接梗提示、互动风格、回复偏好、短摘要和避坑提示）。个性化记忆默认开启、固定保留 90 天；普通用户可关闭个性化学习 / 使用，也可清空全部档案，或对单 UID 重置印象 / 删除档案。见「数据边界」。
...
| `core/recent_output_families.py` | 已交给宿主的 pushed 文本提取、synthetic/dry_run 输出排除、spent-output family 识别与 recent family 窗口统计 | 不判定事件 route，不构造 prompt 文案，不读取直播连接状态；pushed 不证明播放完成 |
...
弹幕 WS → danmaku_core → on_event(LiveDanmaku 富模型)
  ...
  -> handle_live_payload -> pipeline.handle_event:
    ...
    -> solo_stream:
         safety_guard.before_output()    交给宿主前的输出限流
      -> neko_dispatcher.push_roast(respond)
      -> plugin.push_message -> main_server（这里只证明宿主接收，不证明已经播放）
    -> co_stream + auto_low_interrupt:
         safety_guard.before_output()    交给宿主前的输出限流
      -> neko_dispatcher.push_roast(respond, bounded active output)
      -> 宿主现有主动投送队列：priority / FIFO / opt-in coalesce
      -> 插件附带前向兼容 metadata，当前宿主按开放字典安全透传/忽略未知字段
```

投送状态词的语义（详见 `runtime-observability.md`）：
...
同播话权由**宿主在投送时刻**判定，插件不判断也不应读取 VAD、播放或 floor
状态：插件即使查到也只会得到过期快照。#2491 的收束目标只是用户或 AI 正在
发声/播放时暂缓 `respond`，开门后先淘汰过期 cue，再沿用 priority / FIFO /
...
该语义（先备好一句、等下一个自然话轮再说）已从插件生产路径退役。被动语义
只由 `ai_behavior="read"` 保证：只排队，等待自然用户轮次/现有安全消费点，
不主动开口。插件不得依赖宿主识别或降级 `delivery_intent="deferred_candidate"`，
新代码也不得重新引入该 intent。

### 运行时日志

`core/runtime_log.py` 消费 `core/runtime_timeline.py` 已脱敏的记录（HMAC uid、白名单 reason code），不新增数据采集面。dispatcher 结果与失败即时成行，其余计数后每 60 条或会话边界汇总一次；**按记录数而非时钟触发，因此不新增后台任务**。

会话边界即使零事件也会成行：`disconnect` 汇总显示 `records=0` 本身就是诊断结论，用于区分「房间没人说话」与「事件被静默吞掉」。字段语义见 `runtime-observability.md`「Runtime Log」。
...
- `live_events/ritual_memory.py`：会话内、有界（至多 8 条）的房间仪式记忆。它填的是其它回忆机制结构上填不了的空白——`spent_output_family`、recent topic/beat/axis/shape 窗口和宿主 BM25 守卫全部按条数压制近期素材，一个素材掉出定长队列后就彻底不存在，因此没有任何状态表示「够老了、值得再说一次」。晋升要求房间在两个相隔 60 秒以上的独立窗口里自发重复同一信号，且每次至少 2 名不同观众（复用既有 repeated-signal 检测，单人刷屏无法晋升）；猫猫自己的输出永不晋升。回收受三条门禁约束：间隔（回收后锁 180 秒）、换语境（房间主题未变则拒绝）、磨损（用满 3 次退休）。过期看的是**房间最后一次响应**而非猫猫最后一次使用——房间不玩了的梗会死掉，不能由猫猫一个人硬追。梗只进 prompt，绝不进 status/audit。
- `live_events/room_verdict.py`：观众对猫猫提问的集体回答统计。插件本就有整套 `either_or` / `tiny_choice` / `one_word_call` 素材目录，也能识别单条短回答，但此前没有任何地方统计房间选了什么。`pushed` 只表示交给宿主，不表示观众听见；因此当前 choice 形态 handoff 会关闭旧票、保持新票关闭并记录 `delivery_unconfirmed`，直到宿主提供有关联 ID 的播放确认后才允许开票。计票能力本身仍按**每一条弹幕**而非仅被选中回复的那条设计：一人一票（首次答案计数）、至少 2 名不同观众才算房间意见、超过 8 字算聊天不算投票、至多 6 种答案与 64 名投票者、60 秒过期、结果**只宣布一次**。得票 ≥60% 的胜出答案可喂给 `ritual_memory` 作为仪式候选；意见分裂不产生候选。答案 token 只进 prompt。
- `core/runtime_log.py`：脱敏运行日志，见「运行时日志」。
...
- `contribution_rank`：当前版本明确不采集贡献排名；未来只有在产品价值、数据可信度和隐私成本单独获批后才可重新立项。
- `watch_time`：当前版本明确不采集观看时长；不得用不可靠的进房/离房事件伪造停留时间，未来重新立项前须先证明采集可信度。
...
**测试**：`tests/test_credential_store.py`（加解密往返 / 落盘为密文 / 删除）；契约 `test_bili_login_status_without_credential_is_logged_out`、`test_bili_logout_removes_local_credential`、`test_credential_cookie_built_from_credential`，以及 `test_runtime_live_controls.py` 的登录必需、显式受限兜底、登录优先、校验异常、严格布尔值和抖音拒绝用例。
...
流水线故障与宿主投送故障使用相互独立的滑动窗口计数。任一窗口首次达到阈值时只记录一次 `safety_auto_stop` 状态转换；已经在途的请求随后继续失败时仍保留故障计数，但不会重复触发自动停止告警。测试必须通过 `plugin/conftest.py` 的 `patch_module_clock` fixture 绑定 `safety_guard_failures` 自己的时钟，不得替换共享的 `stdlib time.monotonic`。人工恢复会清空故障窗口、队列/降级状态和输出冷却。
...
监控里的 `recent_*` 路由计数表示最近尝试数，包含 skipped / failed；`recent_actual_*` 表示最近 pushed / dry_run 的路由数，即插件已交给宿主或完成 dry-run，不代表播放完成。判断开场暖场、冷场陪播、主动营业是否成功越过插件 dispatcher，以及判断 `proactive_in_engaged`、`active_blocks_idle`、`warmup_repeat`、`warmup_missing`、`idle_missing`、`active_missing`、`avatar_roast_share` 和 `avatar_bias` 时，应优先看 actual 口径，避免把被跳过或失败的尝试误认为已经交给宿主；是否真正出声仍需宿主生命周期或真机证据。`active_blocks_idle` 表示冷场陪播已经 eligible 且 `active_idle_wait` 已归零，但 director 仍选择主动营业，应优先检查主动营业和冷场陪播的让位关系。
...
插件侧仍需要在 prompt、素材选择和 monitor 中处理两类直播现场高风险草稿：一是结尾停在“选 A 还是选 B”这类未完成二选一的问题；二是生成了低置信度教程、游戏攻略、惩罚审判、公开处刑、泛泛喊弹幕互动，或把核电站 / 代码 / 电路 / 泰拉瑞亚等素材硬编成“你是打算...”式含糊技术问句等不适合独播场景的句子。`tools/monitor_live.ps1` 只能把后端日志里的 `neko_live_reply_shape_reason` / `NEKO Live ...` 当成历史兼容或外部实验信号；当前验收以插件 recent result、metadata、真实输出长度和人工复盘为准继续收敛素材和提示词。
...
**读写了哪些用户数据**：中枢不落任何新增用户数据。除窗口胜者外，它在内存里持有最多 12 条时间受限候选，并用最多 3 个引用保留本场最后三条公开弹幕事实（场次序号、UID、脱敏昵称、最多 512 字的脱敏文本、到达时间、是否被选择、是否已被接梗）。三条尾队列不按秒清除，只由新弹幕覆盖；120 秒接梗候选仍按时间淘汰。若 B 站或抖音桥明确提供安全的消息 ID，中枢另持有最多 64 个本场 ID，只用于过滤网络重复投递；不按 UID + 文本生成指纹，因此观众真正重复发送同一句仍保留。直播断开、换房、重连和 teardown 会全量清空这些状态。自定义对象不会被字符串化，异常/非有限/回拨时钟会按本场单调时间收紧，重复 UID + 文本被一次接梗后只在接梗候选视图中整体隐藏，不改写事实尾队列。相关性 `query` 只做当前调用的本地计算，不保存、不进 status；status 只暴露数量、容量、请求/命中/抑制/已用计数和稳定原因，不暴露消息 ID。窗口与原始文本不进入 audit、dashboard、配置、logger 或磁盘。头像不经中枢（弹幕不含头像，由下游 `bili_identity` 按 UID 抓）。档案 / 审计 / 总结的写入仍由既有边界负责。

**测试命令与主要场景**：`plugin/plugins/neko_live/tests/test_live_events.py` 覆盖空闲态首条即时、冷却期开窗按 `get_score` 择优、整窗只投 1 条、本地冷却防并发双锐评、空 uid 只记录事实不投递、空文本丢弃、近期事实容量 / 过期 / 重复文本精确选择、固定 3 条本场尾队列及 30 秒新鲜度标记、仅按稳定投递 ID 去重且保留真实复读、主动胜者不重复进入被动快照、已发布快照在唯一内容转为主动胜者后安全撤销、120 秒接梗视图独立过期、非法对象/时钟/limit 防御、重复弹幕折叠与一次性消费、本地相关性匹配和敏感 query 拒绝、高压隐藏、`reset` 场次清空和 cooldown 时序；`test_active_topic_core.py` / `test_active_topic_selector.py` 覆盖未选中弹幕进入既有主动营业选题并标记已用；`test_plugin_lifecycle.py` 与 `test_runtime_live_controls.py` 覆盖工具角色限定、精确/相关两种模式、后端位置选择、旧消息措辞、连接启停和静态使用规则。B 站与抖音 ingest 测试覆盖显式消息 ID 的安全投影。支持事件接线、优先级、聚合与队列边界由 `test_live_support_scheduler.py` 覆盖。契约测试锁住「富模型 `on_event` → EventBus → 对应单一消费者 → pipeline」打通。

**已知限制**：① `live_events` 只负责普通弹幕择优；支持事件由 `live_support_events` 处理。② 「首评即时」下，空闲态第一条弹幕即使紧随其后到了更高分弹幕也不会被改选——这是用「临场感」换来的，已拍板取舍。③ 窗口择优依赖 `get_score()` 的打分权重（见 `livedanmaku.get_score`），权重调整会改变择优结果。④ 事实问答只覆盖插件在本场实际收到的最后 3 条，不补抓平台历史；三条按数量覆盖而不是按时间清除，断开或换房仍会清空。未选中弹幕不注入每轮普通聊天；在 120 秒内、独播低压时，它只能被一次具体话题相关性查询或既有主动营业选中。相关性是保守的本地词面匹配，会漏掉同义词、隐喻和间接梗；无匹配时必须静默继续当前回答，不能捏造。⑤ 普通聊天是否调用仍由模型决定；不会新增后台调用，但真正调用时可能让该轮增加一次工具往返。成本预算、独立回滚和已批准 Decision Points 见 `docs/modules/live_events.md`。
...
Danmaku Response Review Pack v1 把直播复盘字段收拢到 recent result / monitor / live_explain：`runtime_live_input.expose_request_metadata()` 会安全投影 `danmaku_profile`、`danmaku_reply_target`、`danmaku_reply_shape`、`danmaku_anchor_hint`、`reply_length_mode` 和 `room_theme`，字段只接受真实字符串，不会把 object / bytes / container 字符串化成监控控制字段。`tools/monitor_live.ps1` 对应输出 `latest_reply_length_mode`、`latest_reply_target`、`latest_anchor_hint`、`latest_room_theme` 和 `latest_reply_shape_reason`；其中 `latest_reply_shape_reason` 优先读 hosted-ui recent metadata，backend log 只能作为历史兼容线索，不能要求宿主实现 NEKO Live 专用 output audit。`runtime_dashboard_explain.latest_result` 暴露同一组安全短字段，便于把 trace_id、shape reason 和 room bridge 决策对上。这个包只用于复盘“分类错 / 桥接错 / 插件侧 prompt 或 fallback 设计错”，不新增路由、不改变回复节奏、不暴露原始 room examples。
...
`tools/monitor_live.ps1` 会把这层内容族也暴露给直播复盘：`latest_topic_family` / `latest_host_beat_family` 用于看最近一次主动营业或冷场陪播用了哪类主持手法，`recent_topic_family_*` / `recent_host_beat_family_*` 用于看最近窗口内是否一直停在同一类；当最近主动营业或冷场陪播至少 3 次且同一内容族占比过高时，`alerts` 会出现 `topic_family_bias` 或 `host_beat_family_bias`。这类告警对应的是“听起来像在复读同一种主持套路”，应优先扩素材池或调整 family 轮换，而不是先提高开口频率。
...
普通弹幕回应也要参与同一套已用素材记账：`recent_interaction_context()` 会从已交给宿主的 NEKO 文本中提取 `spent_output_family`，用于标记“小鱼干奖励 / 弹幕接话 / 主播力自测 / 暗号回调”等已经花费过的旧梗。该字段会进入 `recent_results[*].spent_output_family`，`tools/monitor_live.ps1` 会输出 `latest_spent_output_family` 与 `recent_spent_output_family_*`，方便直播现场确认“旧梗 family 是否正在重复”；当最近 handed-off 输出至少 3 次且同一 spent-output family 占比过高时，`alerts` 会出现 `spent_output_family_bias`。dispatcher / dry_run / queued placeholder 摘要不能产生 `spent_output_family`；只有 `pushed` 文本进入记账，但这是一种防复读的保守“已花费”口径，不证明观众听见或播放完成。prompt 侧必须把 `spent_output_family` 和 `topic_family` / `host_beat_family` 一样视为 forbidden material，避免只避开原句却继续复用同一类包袱。`idle_hosting` / `active_engagement` 的素材选择也会优先避开最近已交给宿主的 spent-output family；如果候选全部命中，才逐步放宽到现有 key / title / family fallback，避免直播被防复读规则卡死。英文 family token 必须按完整词或明确短语命中，不能因为 `explain` / `catch` / `presentation` 这类子串误判成 plan / chat / reward。
...
Live Feel Pack v1.12 把 audience-prompt 复读作为单独的高风险主持套路处理。连续输出里如果都在召唤观众“想听 / 想看 / 聊点 / 发言 / 发弹幕 / 接话 / 来一句 / 扣 1 / 吱一声 / 冒个泡 / 给点反应 / 还在吗 / 有人吗 / 在不在 / 打个分”，即使句子不相似，也应视为 `audience_prompt` family 的换皮复读，并进入插件侧复盘。`pushed` 后交给宿主的文本中出现这些表达时，也会被 `_spent_output_families()` 标成 `audience_prompt`，进入 `recent_interaction_context()`、`spent_output_family`、素材选择避让和 monitor 复盘口径；该记账仍不等于播放完成证据。普通“弹幕”一词本身不能单独作为强复读信号，避免把正常接弹幕误判成模板营业；需要命中更明确的观众召唤动作。
...
1. 历史空壳 `bili_dm_ingest`、`contribution_rank`、`watch_time`、`bili_read_tools`、`bili_write_tools`、`automation_ops` 已在 2026-07-15 完成消费者审计后移除：它们只有 `ENABLED = False` 占位和 Dashboard 注册，没有 action、配置、UI、测试或兼容导入依赖。`ReservedModule` 类型本身继续保留，用于真实模块导入失败时的安全降级和 registry 隔离测试；不得再为“未来可能需要”的功能预建空目录或 Dashboard 假模块。
...
- “清空观众档案”只清 `viewer_profiles.json` 中的观众档案，不清空 `recent_results`、沙盒记录、直播总结或安全队列。单 UID 治理动作分两类：`delete_viewer_profile` 删除该 UID 的整条持久档案并清掉该 UID 的 session 首评标记，下一次互动按新观众处理；`reset_viewer_impression` 只清 `preference_tags` / `favorite_topics` / `running_jokes` / `interaction_style` / `response_preference` / `impression_summary` / `avoid_guidance` 等印象字段，保留 UID、昵称、计数、首次出场记录和本场 session 状态，用于修正错误印象而不重新触发首评。
- 观众档案清空、单 UID 印象重置和单 UID 删除是普通用户的数据权利入口，不再依赖 `developer_tools_enabled`；Hosted UI 必须使用明确文案和二次确认，runtime / store 必须返回 `applied` / `deleted` / `reset` 等真实结果，不能只靠前端按钮状态假装成功。手工模拟弹幕、身份调试等开发工具仍必须在后端检查开发者模式。修改这些入口时必须同步 `ui/panel.tsx`、`ui/panel_data_sections.tsx` 和完整兼容入口 `ui/panel_compat.tsx`，并保留 smoke gate。
...
界面分为**四个普通一级页**，开发者模式开启后条件追加 `dev`；id / 顺序固定为 `console / interaction / viewers / settings / dev?`，由面板契约测试锁住：

- `控制台 console`：正常直播的完整操作入口。账号与平台、直播间两步查询 / 确认、开播结论、节奏设置、本场状态和底部唯一“开始 / 结束直播”主动作都在这里完成。主按钮必须按账号与直播间准备状态 fail closed；`dry_run`、trace、模块清单和 raw safety 状态不得回流普通控制台。
- `直播间互动 interaction`：首次出场锐评、后续弹幕接话、Gift / SC / Guard 致谢、开场暖场、冷场陪播和主动营业。每个模块保留稳定摘要卡，详细设置使用弹窗；功能关闭时卡片高度和整体网格不应剧烈跳动。
- `观众 viewers`：内部固定为“本场直播 / 观众档案”。本场页展示有界会话统计与最近互动观众；档案页展示精简摘要和详情弹窗。`live_explain`、recent pipeline result、trace 和内部 route 只进入开发者“运行结果”。
- `设置 settings`：内部固定为“安全与性能 / 数据与隐私 / 帮助与高级”。普通设置只保留行为级选项、存储状态、新手教程重置和开发者模式开关；`dry_run`、精确内部阈值、audit 与模块清单不作为普通用户设置。
- `开发者工具 dev`：仅 `developer_tools_enabled=true` 时出现，内部固定为“身份与头像查询 / 模拟直播事件 / 运行结果”。权限以后端检查为准，关闭开发者模式必须受控退出当前页。

**「一张嘴」切分**：功能级参数（开关 / 强度 / 去重…）跟功能走、进「直播间互动」功能卡；日常节奏与模式贴近控制台，安全 / 性能 / 隐私偏好留设置，`dry_run` 与技术诊断留开发者工具。`live_enabled` 是直播监听与输出总闸，单一真相源为控制台底部的开始 / 结束直播动作，不再伪装成首次锐评模块开关。互动页公开七个持久化功能开关：`avatar_roast_enabled`、`avatar_analysis_enabled`、`danmaku_response_enabled`、`live_support_events_enabled`、`warmup_hosting_enabled`、`idle_hosting_enabled`、`active_engagement_enabled`；默认均开启以保持既有行为。关闭后必须在对应 runtime / pipeline 入口 fail-closed，其中头像分析关闭时不得下载头像字节，三类自动主持关闭时手动和自动触发都不得进入 pipeline。
...
已**选择性复用** `bilibili_danmaku` 的直播主链路：吞并其连接+解析层（`danmaku_core` / `livedanmaku` → `bili_live_ingest`）、移植 `bili_auth_service`（扫码登录 → `adapters`），并修了搬来的 `from_danmaku` `info[7]` bug；NEKO Live 没有直接沿用其 LLM / orchestrator / memory，而是走统一人设、EventBus、pipeline 与安全门。迁入代码现已独立维护，NEKO Live 不导入旧插件包；但旧插件仍保留平台内容读写、历史查询、用户管理和独立分析等未完成取舍的能力，尚未达到最终退役门槛。
